...
from .lab import BoundaryLab, create_lab
...
def _init_git(workspace: Path) -> None:
    try:
        subprocess.run(["git", "init", "-q"], cwd=workspace, capture_output=True, timeout=5, check=False)
...
def verify(
    ...
    *,
    ...
    lab = create_lab(network_probe=network_probe)
